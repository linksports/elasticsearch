...
type Elasticsearch interface {
	...
	Search(index string, query string, data any) (StatusCode, []*HitData, int, error)
	SearchWithAggregations(index string, query string, data any, aggsKeys []string) (StatusCode, []*HitData, int, map[string][]*AggregationBucket, error)
			...
			for i, item := range items {
				if itemMap, ok := item.(map[string]any); ok {
					for _, data := range itemMap {
						if indexData, ok := data.(map[string]any); ok {
							if indexError, ok := indexData["error"].(map[string]any); ok {
...
func (es *_elasticsearch) Search(index string, query string, data any) (StatusCode, []*HitData, int, error) {
...
func (es *_elasticsearch) SearchWithAggregations(index string, query string, data any, aggsKeys []string) (StatusCode, []*HitData, int, map[string][]*AggregationBucket, error) {
	...
	if len(aggsKeys) > 0 {
		for _, key := range aggsKeys {
			if _, ok := result["aggregations"].(map[string]any)[key]; !ok {
				continue
			}

			buckets := result["aggregations"].(map[string]any)[key].(map[string]any)["buckets"].([]any)
			var aggregationBucket []*AggregationBucket
			for _, bucket := range buckets {
				b := &AggregationBucket{
					Key:      bucket.(map[string]any)["key"],
					DocCount: bucket.(map[string]any)["doc_count"].(float64),
				}
...
func search(client *goElasticsearch.Client, index string, query string) (StatusCode, map[string]any, error) {
...
func parseHitsData(result map[string]any, data any) (StatusCode, []*HitData, int, error) {
	if _, ok := result["hits"]; !ok {
		return StatusNoContent, []*HitData{}, 0, nil
	}

	total := 0
	if t, existsTotal := result["hits"].(map[string]any)["total"]; existsTotal {
		total = int(t.(map[string]any)["value"].(float64))
	}

	hits := result["hits"].(map[string]any)["hits"].([]any)

	documents := make([]any, len(hits))
	hitsData := make([]*HitData, len(hits))

	for i, hit := range hits {
		documents[i] = hit.(map[string]any)["_source"]

		h := &HitData{
			Index: hit.(map[string]any)["_index"].(string),
			Id:    hit.(map[string]any)["_id"].(string),
		}

		if score := hit.(map[string]any)["_score"]; score != nil {
			h.Score = score.(float64)
		}

		if sort := hit.(map[string]any)["sort"]; sort != nil {
			h.Sort = sort.([]any)
		}
